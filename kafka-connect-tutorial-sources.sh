...
# Linux / Mac
docker run --rm -it --platform linux/arm64 -v "$(pwd)":/tutorial --net=host landoop/fast-data-dev bash
...
docker run --rm -it --platform linux/arm64 --net=host landoop/fast-data-dev bash
